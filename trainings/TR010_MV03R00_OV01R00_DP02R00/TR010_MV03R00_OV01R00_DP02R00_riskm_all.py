...
def load_keras_model(model_file_name):
    model = load_model(os.path.join(RMC.OUTPUT_DIR, model_file_name + '_model.h5'))
...
def execute_test(fpp, model, test_x, test_y, model_dir, model_file_name):
    ...
    test_result = pd.DataFrame(
        {RMC.SIM_ID_COL: test_x.index.levels[0], 'y': y, 'y_pred': y_p, 'Difference': y - y_p, 'Deviation': (y - y_p) * 100 / y})
    test_result.set_index(RMC.SIM_ID_COL, inplace=True)
    test_result.sort_index(inplace=True)
...
def main():
    ...
    if train or test:
        ...
        if RMC.LOAD_MODEL is not None:
            logger.info("Loading model ...")

            fpp = load_feature_prep_pipeline(RMC.LOAD_MODEL)
            model = load_keras_model(RMC.LOAD_MODEL)
